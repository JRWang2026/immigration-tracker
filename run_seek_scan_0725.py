#!/usr/bin/env python3
"""
SEEK NZ 绿名单 Tier1 聚焦扫描 — 2026-07-25 批次
读取 QQ Mail MCP GetMessage 工具结果文件，提取岗位，评分，生成报告 + KOS JSON。
"""
...
DATA_DIR = WORKSPACE / "data" / "seek-nz"

# 工具结果文件目录（当前 MCP GetMessage 输出）
TOOL_RESULTS_DIR = Path("C:/Users/Mr_Wang/.workbuddy/projects/c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36/a15186a8-bb44-4eca-a7a1-c2fe643c11d1/tool-results")

# ---------------------------------------------------------------------------
# 邮件正文提取
# ---------------------------------------------------------------------------
def extract_body_from_tool_result(filepath: Path) -> Tuple[str, dict]:
    """从 QQ Mail connector 工具结果文件中提取 body HTML 和邮件元数据。"""
    ...
    try:
        ...
        meta = {
            ...
            "from": msg.get("from", {}).get("email", "") if isinstance(msg.get("from"), dict) else str(msg.get("from", "")),
...
def extract_jobs_from_html(body: str) -> List[Dict[str, str]]:
    ...
    for card in cards[1:]:
        ...
        jobs.append({
            ...
            "salary": html_module.unescape(salary),
...
def main():
    ...
    if TOOL_RESULTS_DIR.exists():
        for f in sorted(TOOL_RESULTS_DIR.glob("mcp-connector-proxy-qq-mail_GetMessage-*.txt")):
            body, meta = extract_body_from_tool_result(f)
            if len(body) > 500:
                jobs = extract_jobs_from_html(body)
                if jobs:
                    all_raw_jobs.extend(jobs)
                    email_count += 1
                    subject = meta.get("subject", f.name)
                    email_subjects.append(subject)
                    print(f"  文件 {f.name}: 提取 {len(jobs)} 个岗位 | {subject[:60]}")
                else:
    ...
    # 判断是否为绿名单 Tier1 岗位（基于 ANZSCO，不只看分数）
    def is_green_list_tier1(job: dict) -> bool:
        return job["anzsco_code"] in GREEN_LIST_TIER1

    # 4. 分类统计
    tier1 = [j for j in unique_jobs if is_green_list_tier1(j)]
    research = [j for j in unique_jobs if 40 <= j["score"] < 60 and not is_green_list_tier1(j)]
    others = [j for j in unique_jobs if j["score"] < 40 and not is_green_list_tier1(j)]

    tier1_count = len(tier1)
    print(f"\n分类统计：")
    print(f"  绿名单 Tier1: {len(tier1)}")
    ...
    email_summary = "、".join([s[:50] for s in email_subjects]) if email_subjects else "无"
    ...
    lines = [
        ...
        f"> 邮件主题：{email_summary}",
        "",
        "---",
        "",
        "## 一、绿名单 Tier1 ICT 岗位（Straight to Residence）",
        "",
    ]
    ...
    if tier1:
        ...
        for j in tier1:
            ...
            marker = "🔥" if j["score"] >= 60 else ""
            lines.append(
                f"| **{j['score']}** {marker}| [{j['title']}]({j['url']}) | {j['company']} | {j['location']} | {j['salary']} | {anzsco} | {j['immigration_path']} | {j['suggested_skills']} |"
            )
    else:
        lines.append("*今日无绿名单 Tier1 ICT 岗位。*")
    ...
    lines.extend([
        ...
        "## 四、操作摘要",
        "",
        f"- 绿名单 Tier1：{len(tier1)}",
        f"- 大学/研究岗（40-59分）：{len(research)}",
        f"- 已过滤/降级（<40分）：{len(others)}",
        "",
        "**下一步建议**：",
    ...
    feed = {
        ...
        "data": {
            ...
            "email_count": email_count,
            "total_jobs": len(unique_jobs),
            "tier1_jobs": [build_job_record(j) for j in tier1],
    ...
    for i, j in enumerate(unique_jobs[:10], 1):
        tier_marker = "🥇" if is_green_list_tier1(j) else "🥈" if j["score"] >= 40 else ""
