...
BASE = r'C:\Users\Mr_Wang\.workbuddy\projects\c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36\f38e82b8-bb22-4ecd-8a61-69cda868c87f\tool-results'
EMAIL_FILES = [
    (os.path.join(BASE, 'mcp-connector-proxy-qq-mail_GetMessage-1785550163814-1501ea.txt'), 'json', 'Admin 8/01'),
    (os.path.join(BASE, 'mcp-connector-proxy-qq-mail_GetMessage-1785550164355-8d1da5.txt'), 'json', 'ICT 7/31 22:47'),
    (os.path.join(BASE, 'mcp-connector-proxy-qq-mail_GetMessage-1785550164910-23cb87.txt'), 'json', 'NZ General 7/31'),
    (os.path.join(BASE, 'mcp-connector-proxy-qq-mail_GetMessage-1785550165418-11ba3e.txt'), 'json', 'ICT 7/31 21:25'),
]
...
email_labels = 'Admin 8/01 + ICT 7/31 x2 + NZ General 7/31'

report = f"""# SEEK NZ 岗位扫描报告 - {today} (绿名单Tier1聚焦版)

> 📅 扫描时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 📧 来源：QQ邮箱SEEK推送（4封邮件，{email_labels}）
