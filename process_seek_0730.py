#!/usr/bin/env python
"""SEEK NZ 岗位扫描 - 2026-07-30 处理脚本
读取 QQ Mail MCP GetMessage 工具结果文件，MJML模板解析，绿名单Tier1聚焦评分。
8封邮件：7/28 (Admin+ICT+NZ General+ICT13) + 7/29 (Admin+ICT+NZ General+ICT17)
"""
...
# === File paths ===
TOOL_RESULTS_DIR = Path(r"C:\Users\Mr_Wang\.workbuddy\projects\c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36\36784eec-b585-49cd-9c1d-aefa8fee1097\tool-results")

FILES = [
    # 7/29 emails
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370544943-c295fd.txt", "json", "7/29 Admin"),
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370545457-194004.txt", "json", "7/29 ICT"),
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370546001-e89dbf.txt", "json", "7/29 NZ General"),
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370546507-e095df.txt", "json", "7/29 ICT2"),
    # 7/28 emails
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370547000-66243e.txt", "json", "7/28 Admin"),
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370547502-43eada.txt", "json", "7/28 ICT"),
    (TOOL_RESULTS_DIR / "mcp-connector-proxy-qq-mail_GetMessage-1785370548016-288288.txt", "json", "7/28 NZ General"),
    # 7/28 ICT 13 -- body injected inline from GetMessage response
    ("__INLINE_ICT13__", "inline", "7/28 ICT3"),
]

# The inline body will be set by the runner
INLINE_ICT13_BODY = None


def load_body(path, ftype):
    if ftype == "inline":
        if INLINE_ICT13_BODY is None:
            raise RuntimeError("INLINE_ICT13_BODY not set!")
...
for path, ftype, label in FILES:
    ...
    email_count += 1
    file_processed_labels.append(label)
    fname = os.path.basename(str(path)) if ftype != 'inline' else 'inline_ICT13'
    print(f"[{label}] {fname}: {len(jobs)} jobs extracted")
...
print(f"Labels: {', '.join(file_processed_labels)}")
...
tier1_jobs_all = [j for j in unique_jobs if is_green_list_tier1(j['title'])]
tier1_count = len(tier1_jobs_all)
...
> 📅 扫描时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 📧 来源：QQ邮箱SEEK推送（{email_count}封邮件，7/28-7/29 Admin×2 + ICT×4 + NZ General×2）
...
| 指标 | 数值 |
|------|------|
| 扫描邮件数 | {email_count}（7/28 Admin×1 + ICT×3 + NZ General×1 + 7/29 Admin×1 + ICT×2 + NZ General×1） |
...
| 7/28 | 4 | 58 | 0 | ➡ 继续归零 |
| 7/29 | 4 | 42 | 0 | ➡ 继续归零 |
| **7/30** | **{email_count}** | **{len(unique_jobs)}** | **{tier1_count}** | **{'⬆' if tier1_count > 0 else '➡ 继续归零'}** |
...
kos_data = {
    ...
    "tier1_jobs": [build_job_record(j) for j in tier1_jobs_all],
...
print(f"Tier1 jobs: {[j['title'] + ' (' + j['company'] + ')' for j in tier1_jobs_all]}")
print(f"Top 10 scores: {[(j['title'], j['company'], j['score']) for j in unique_jobs[:10]]}")
print(f"{'='*60}")
