"""
SEEK NZ 扫描 - 2026-08-06 (绿名单Tier1聚焦版)
- 4封邮件：08/05 Admin 20 jobs + ICT 20 jobs + NZ General 20 jobs + ICT 14 jobs
- 文件来源：QQ Mail MCP GetMessage 工具结果 JSON
"""
...
# 4 file-based emails from 2026-08-05
TOOL_RESULTS_DIR = r'C:\Users\Mr_Wang\.workbuddy\projects\c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36\c1e04045-6249-4280-a296-829487d213a1\tool-results'
file_paths = [
    (os.path.join(TOOL_RESULTS_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1785975146371-a677b3.txt'), 'json'),   # Admin 20 jobs
    (os.path.join(TOOL_RESULTS_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1785975146771-7b3db3.txt'), 'json'),   # ICT 20 jobs
    (os.path.join(TOOL_RESULTS_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1785975147201-171952.txt'), 'json'),  # NZ General 20 jobs
    (os.path.join(TOOL_RESULTS_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1785975147600-3b348b.txt'), 'json'),  # ICT 14 jobs
]
...
> 📅 扫描时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 📧 来源：QQ邮箱SEEK推送（4封邮件：8/5 Admin + 8/5 ICT×2 + 8/5 NZ General）
...
| 指标 | 数值 |
|------|------|
| 扫描邮件数 | 4（8/5 Admin + 8/5 ICT×2 + 8/5 NZ General） |
...
# Track jobs from previous rounds if they appear again
tracking = []
...
# Check if previous tracked jobs still appear
for title, company, first_seen, status in [
    ('SRE / DevOps Engineer', 'Walker Smith', '2026-08-04', '绿名单Tier1 DevOps/SRE'),
    ('Online Research Candidates', 'PanelPlace', '2026-08-03', '研究型'),
    ('Senior IT Support Engineer', 'Computer Clinic', '2026-08-04', '绿名单Tier2近'),
]:
    sc = current_score(title, company)
    if sc != '-':
        tracking.append((title, company, first_seen, sc, status + '（持续）'))

# Also add any new tier1 jobs from this round
for j in unique_jobs:
    if is_green_list_tier1(j['title']) and not any(t[0].lower() in j['title'].lower() for t in tracking):
        tracking.append((j['title'], j['company'], '2026-08-05', j['score'], '🆕绿名单Tier1'))

if not tracking:
    report += '| (无持续跟踪岗位) | | | | |\n'
else:
    for t in tracking:
        sc_val = t[3] if isinstance(t[3], str) else f"{t[3]}"
        report += f"| {t[0]} | {t[1]} | {t[2]} | {sc_val} | {t[4]} |\n"
...
### 主线不变：德国岗位制博士（90%精力）
- 当前SEEK推送中绿名单Tier1 ICT岗密度极低（连续多日Tier1归零/极少）
...
email_count = sum(1 for path, _ in file_paths if os.path.exists(path))
