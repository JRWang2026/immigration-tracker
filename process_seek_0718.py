#!/usr/bin/env python3
"""
SEEK NZ Job Scanner - 2026-07-18
Processes 7 SEEK job alert emails from 7/16 + 7/17
...
BASE_DIR = r'C:\Users\Mr_Wang\.workbuddy\projects\c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36\d334d035-1a10-4b6f-9662-84dee0d6116e\tool-results'

EMAIL_FILES = [
    # 7/17 batch (newest)
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335407688-a73aef.txt'),
     '2026-07-17 ICT 20jobs (22:47)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335407835-a65182.txt'),
     '2026-07-17 NZ General 20jobs (21:28)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335407757-03fbc2.txt'),
     '2026-07-17 ICT 20jobs (21:25)'),
    # 7/16 batch (not yet reported)
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335407617-58727c.txt'),
     '2026-07-16 Admin 20jobs (23:58)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335416592-2c074f.txt'),
     '2026-07-16 ICT 20jobs (21:47)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335416433-657aba.txt'),
     '2026-07-16 NZ General 20jobs (20:28)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784335416520-835c32.txt'),
     '2026-07-16 ICT 15jobs (20:25)'),
]
...
consecutive_zero = 8 if tier1_count == 0 else 0  # 7/10 was last Tier1

report = f"""# SEEK NZ 岗位扫描报告 - {today} (绿名单Tier1聚焦版)

> 扫描时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 来源：QQ邮箱SEEK推送（{email_count}封邮件，7/17 ICTx2 + NZ General + 7/16 ICTx2 + Admin + NZ General）
...
| 指标 | 数值 |
|------|------|
| 扫描邮件数 | {email_count}（7/17 ICTx2 + NZ General + 7/16 ICTx2 + Admin + NZ General） |
