#!/usr/bin/env python3
"""
SEEK NZ Job Scanner - 2026-07-20
Processes 9 SEEK job alert emails from 7/18 + 7/19
...
BASE_DIR = r'C:\Users\Mr_Wang\.workbuddy\projects\c-Users-Mr_Wang-WorkBuddy-2026-06-20-14-48-36\75b5bf44-eb8c-425d-9ea3-4427c75669be\tool-results'

EMAIL_FILES = [
    # 7/19 batch (newest, unread)
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329556-e3ee7a.txt'),
     '2026-07-19 Admin 20jobs (23:58)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506330153-eeb758.txt'),
     '2026-07-19 ICT 20jobs (21:47)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506330234-329b71.txt'),
     '2026-07-19 NZ General 20jobs (20:28)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506330078-35d894.txt'),
     '2026-07-19 ICT 14jobs (20:25)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329901-ad4501.txt'),
     '2026-07-19 Admin 20jobs (00:58)'),
    # 7/18 batch (read but not yet processed)
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329737-7c1dbf.txt'),
     '2026-07-18 ICT 20jobs (22:47)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329661-f29077.txt'),
     '2026-07-18 NZ General 20jobs (21:28)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329821-e7a066.txt'),
     '2026-07-18 ICT 18jobs (21:25)'),
    (os.path.join(BASE_DIR, 'mcp-connector-proxy-qq-mail_GetMessage-1784506329984-12c5d0.txt'),
     '2026-07-18 Admin 20jobs (00:58)'),
]
...
def extract_jobs_mjml(body):
    ...
    for card in cards[1:]:
        # Title: text-decoration:underline pattern (MJML template)
...
# Track green list consecutive zero days
consecutive_zero = 9 if tier1_count == 0 else 0  # 7/10 was last Tier1

report = f"""# SEEK NZ 岗位扫描报告 - {today} (绿名单Tier1聚焦版)

> 扫描时间：{datetime.now().strftime('%Y-%m-%d %H:%M')} | 来源：QQ邮箱SEEK推送（{email_count}封邮件，7/19 Admin×2 + ICT×2 + NZ General×2 + 7/18 Admin×1 + ICT×2 + NZ General×1）
...
| 指标 | 数值 |
|------|------|
| 扫描邮件数 | {email_count}（7/19 Admin×2 + ICT×2 + NZ General×2 + 7/18 Admin×1 + ICT×2 + NZ General×1） |
